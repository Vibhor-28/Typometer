const paraEl = document.querySelector(".para"),
  timeHead = document.querySelector(".time"),
  cw = document.querySelector(".cw"),
  cwstat = document.querySelector(".cw-stat"),
  inputEl = document.querySelector(".input"),
  modepro = document.querySelector(".mode-pro"),
  modebegin = document.querySelector(".mode-begin"),
  time_30 = document.querySelector(".time-30"),
  time_60 = document.querySelector(".time-60"),
  time_stat = document.querySelector(".time-stat"),
...
// const paragraphs2 = [
//   "winston is one of the most laid-back people i know he is tall and slim with black hair and he always wears a t-shirt and black jeans his jeans have holes in them and his baseball boots are scruffy too he usually sits at the back of the class and he often seems to be asleep however when the exam results are given out he always gets an i don't think hes as lazy as he appears to be",
// ];

function loadpara(paragraphs) {
  let random = Math.floor(Math.random() * paragraphs.length);
  paraEl.textContent = paragraphs[random];
  spanTag = "";
  paragraphs[random].split("").forEach((element) => {
    spanTag += `<span>${element}</span>`;
  });

  paraEl.innerHTML = spanTag;

  paraEl.querySelectorAll("span")[0].classList.add("active");
  document.addEventListener("keydown", () => inputEl.focus());
  paraEl.addEventListener("click", () => inputEl.focus());
}
loadpara(paragraphs);

time_30.addEventListener("click", () => {
  time_30.classList.add("yellow");
  time_60.classList.remove("yellow");
  time_stat.textContent = "30s";
  time = 30;
  
});
time_60.addEventListener("click", () => {
  time_60.classList.add("yellow");
  time_30.classList.remove("yellow");
  time_stat.textContent = "60s";
  time = 60;
  maxtime = 60;
});
modepro.addEventListener("click", () => {
  modepro.classList.add("yellow");
  modebegin.classList.remove("yellow");
  loadpara(paragraphs);
});
modebegin.addEventListener("click", () => {
  modebegin.classList.add("yellow");
  modepro.classList.remove("yellow");

  loadpara(paragraphs);
});

resetEl.addEventListener("click", resetgame);
inputEl.addEventListener("input", inputTyping);

function inputTyping() {
  document.querySelector(".parent").classList.add("hidden");
  let characters = paraEl.querySelectorAll("span");
  let user_input = inputEl.value;
  if (charindex < characters.length - 1 && time >= 0) {
    ...
    if (user_input[charindex] == null) {
      if (charindex > 0) {
        charindex--;
      }
      characters[charindex].classList.remove("correct", "incorrect");
    } else {
      if (characters[charindex].textContent === user_input[charindex]) {
...
function init_timer() {
  if (time > 0) {
    time--;
    time_stat.textContent = time + "s";
  } else {
    wpm = Math.round(((charindex - mistakes) / 5 / (maxtime - time)) * 60);
    wpm = wpm < 0 || !wpm || wpm === Infinity ? 0 : wpm;
    timeHead.textContent = "WPM";
    time_stat.textContent = wpm;
    cw.classList.remove("hidden");
    cwstat.classList.remove("hidden");
    let accuracy = Math.round(((correct - mistakes) / charindex) * 100);
    cwstat.textContent = accuracy > 0 ? accuracy + " %" : 0 + " %";
    document.querySelector(".stats").classList.add("text-center");
    document.querySelector(".stats").classList.remove("text-left");
    clearInterval(timer);
  }
}
...
function resetgame() {
  clearInterval(timer);
  loadpara(paragraphs);
  document.querySelector(".stats").classList.add("text-left");
  document.querySelector(".stats").classList.remove("text-center");
  time = 30;
  charindex = 0;
  typing = false;
  inputEl.value = "";
  timeHead.textContent = "Time";
  time_stat.innerText = time + "s";
  cw.classList.add("hidden");
  cwstat.classList.add("hidden");
  time_30.classList.add("yellow");
  time_60.classList.remove("yellow");
  modebegin.classList.add("yellow");
  modepro.classList.remove("yellow");
